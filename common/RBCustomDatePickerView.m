...
- (void)setTimeBroadcastView
{
    ...
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%ld月%ld日 %@ %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    [self addSubview:nowPickerShowTimeLabel];
    
    timeBroadcastView = [[UIView alloc] initWithFrame:CGRectMake(20, 5, 278.5, 190.0)];
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    {
        l.text = [NSString stringWithFormat:@"%i月",1+index];
    }
    else if (scrollView == dayScrollView)
    {
...
- (void)selectSetBroadcastTime
{
    ...
    int yearInt = (int)yearLabel.tag + 1999;
    int monthInt = (int)monthLabel.tag;
    int dayInt = (int)dayLabel.tag;
    int hourInt = (int)hourLabel.tag - 1;
    int minuteInt = (int)minuteLabel.tag - 1;
    int secondInt = (int)secondLabel.tag - 1;
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSLog(@"%@  %@",self.date,selectTimeString);
